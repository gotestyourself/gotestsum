...
func Write(out io.Writer, exec *testjson.Execution, cfg Config) (JUnitTestSuites, error) {
	tests := generate(exec, cfg)
	err := errors.Wrap(write(out, tests), "failed to write JUnit XML")
	return tests, err
}
